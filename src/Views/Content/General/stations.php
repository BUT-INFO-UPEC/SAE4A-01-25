...
<table id="stations_table" class="display table table-striped table-hover">
    <thead>
        <tr>
            ...
            <th>Département<br><select id="filter_dept" class="form-select"><option value="">Tous</option></select></th>
            <th>Région<br><select id="filter_region" class="form-select"><option value="">Toutes</option></select></th>
        </tr>
    </thead>
    <tbody>
        <?php foreach ($stations as $item): ?>
            <tr onclick="window.location.href='<?= BASE_URL ?>?controller=ControllerGeneral&action=info_station'">
